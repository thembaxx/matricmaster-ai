...
import { closeConnection, getDb } from '../src/lib/db';
import { pastPapers } from '../src/lib/db/schema';
import {
	convertPdfToMarkdown,
	uploadMarkdownToUploadThing,
} from '../src/services/markdownConverter';
...
async function migrate() {
	...
	try {
		...
		const papers = await db
			.select()
			.from(pastPapers)
			.where(and(isNotNull(pastPapers.storedPdfUrl), isNull(pastPapers.markdownFileUrl)));
		...
		for (const paper of papers) {
			try {
				...
				if (result.success && result.markdown) {
					console.log(
						`✅ Converted ${paper.paperId} to markdown (${result.markdown.length} chars)`
					);
