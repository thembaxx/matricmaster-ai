...
async function main() {
  ...
  try {
    ...
    for (const persona of personas) {
      const existing = await db.select().from(tUsers).where(eq(tUsers.email, persona.email)).limit(1);
    ...
    for (const persona of personas) {
      ...
      const [deck] = await db.insert(tFlashcardDecks).values({
        id: crypto.randomUUID(),
        userId,
      ...
      for (let i = 0; i < 5; i++) {
        const [card] = await db.insert(tFlashcards).values({
          id: crypto.randomUUID(),
          deckId: deck.id,
          front: `Concept ${i + 1}`,
          back: `Answer ${i + 1}`,
    ...
    const [channel] = await db.insert(tChannels).values({
      id: crypto.randomUUID(),
