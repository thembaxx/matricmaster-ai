...
def migrate_file(file_path):
    ...
    if (
        "from 'lucide-react'" not in content
        and 'from "lucide-react"' not in content
        and "from '@phosphor-icons/react'" not in content
        and 'from "@phosphor-icons/react"' not in content
    ):
        return
    ...
    for lucide, phosphor in mapping.items():
        ...
        content = re.sub(
            rf"import \{{(.*?)\b{lucide}\b(.*?)\}} from \'@phosphor-icons/react\';",
            rf"import {{\1{phosphor}\2}} from '@phosphor-icons/react';",
            content,
            flags=re.DOTALL,
        )
        content = re.sub(
        ...
        content = re.sub(rf"<{lucide}\b", f"<{phosphor}", content)
        content = re.sub(rf"</{lucide}>", f"</{phosphor}>", content)
        ...
        content = re.sub(rf"icon:\s*{lucide}\b", f"icon: {phosphor}", content)
        content = re.sub(rf"Icon:\s*{lucide}\b", f"Icon: {phosphor}", content)
    ...
    for icon in bold_icons:
        ...
        content = re.sub(
            rf'<{icon}(.*?)weight="bold"(\s+weight="bold")+',
            rf'<{icon}\1weight="bold"',
            content,
        )
    ...
    if "DesktopSidebar.tsx" in file_path:
        ...
        match = re.search(
            r"import \{(.*?)\} from \'@phosphor-icons/react\';", content, re.DOTALL
        )
        if match:
