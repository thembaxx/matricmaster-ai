...
type DevEnv = z.infer<typeof devSchema>;

let validatedEnv: Env | null = null;
let devValidatedEnv: DevEnv | null = null;

export function validateEnv(): Env | DevEnv {
	if (validatedEnv) return validatedEnv;
	if (devValidatedEnv) return devValidatedEnv;
	...
	if (!result.success) {
		...
		devValidatedEnv = devSchema.parse({
			...process.env,
			NEXT_PUBLIC_APP_URL: process.env.NEXT_PUBLIC_APP_URL || 'http://localhost:3000',
			NODE_ENV: process.env.NODE_ENV || 'development',
		});
		return devValidatedEnv;
	}
