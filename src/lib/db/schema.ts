...
export const quizResults = pgTable(
	'quiz_results',
	{
		...
		wrongAnswersProcessed: boolean('wrong_answers_processed').notNull().default(false),
	},
	(table) => ({
...
export const flashcardDecks = pgTable(
	'flashcard_decks',
	{
		...
		autoGeneratedFrom: varchar('auto_generated_from', { length: 50 }),
		createdAt: timestamp('created_at').defaultNow(),
		updatedAt: timestamp('updated_at').defaultNow(),
	},
	(table) => ({
...
// ============================================================================
// LEARNING LOOP EVENTS TABLE
// ============================================================================

export const learningLoopEvents = pgTable(
	'learning_loop_events',
	{
		...
		flashcardId: uuid('flashcard_id'),
		triggeredBy: varchar('triggered_by', { length: 20 }).notNull().default('learning_loop'),
		outcome: varchar('outcome', { length: 20 }).notNull().default('pending'),
		createdAt: timestamp('created_at').defaultNow(),
	},
	(table) => ({
		userIdIdx: index('learning_loop_events_user_id_idx').on(table.userId),
		quizIdIdx: index('learning_loop_events_quiz_id_idx').on(table.quizId),
		questionIdIdx: index('learning_loop_events_question_id_idx').on(table.questionId),
		aiSessionIdIdx: index('learning_loop_events_ai_session_id_idx').on(table.aiSessionId),
		flashcardIdIdx: index('learning_loop_events_flashcard_id_idx').on(table.flashcardId),
		triggeredByIdx: index('learning_loop_events_triggered_by_idx').on(table.triggeredBy),
		createdAtIdx: index('learning_loop_events_created_at_idx').on(table.createdAt),
	})
);

export type LearningLoopEvent = typeof learningLoopEvents.$inferSelect;
export type NewLearningLoopEvent = typeof learningLoopEvents.$inferInsert;
