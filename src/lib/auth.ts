...
function createAuth(): AuthInstance {
	...
	const fromEmail = process.env.FROM_EMAIL || 'MatricMaster <noreply@matricmaster.ai>';
	...
	const sendVerificationEmail = async ({
		...
		try {
			await resend.emails.send({
				from: fromEmail,
				to: email,
				subject: 'Verify your MatricMaster account',
				html: `
					<!DOCTYPE html>
					<html>
					<head>
					...
					<body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; background-color: #f5f5f5; padding: 20px;">
						<div style="max-width: 500px; margin: 0 auto; background: white; border-radius: 12px; padding: 32px; box-shadow: 0 2px 8px rgba(0,0,0,0.1);">
							<div style="text-align: center; margin-bottom: 24px;">
								<h1 style="color: #2563eb; font-size: 28px; margin: 0;">MatricMaster</h1>
							</div>
							<h2 style="color: #1f2937; font-size: 20px; margin-bottom: 16px;">Verify your email address</h2>
							<p style="color: #4b5563; font-size: 16px; line-height: 1.6; margin-bottom: 24px;">
								Hi${user.name ? ` ${user.name}` : ''}, welcome to MatricMaster! Please verify your email address to get started with your exam preparation.
							</p>
	...
	const sendPasswordResetEmail = async ({ email, url }: { email: string; url: string }) => {
		...
		try {
			await resend.emails.send({
				from: fromEmail,
				to: email,
				subject: 'Reset your MatricMaster password',
				html: `
					<!DOCTYPE html>
					<html>
					<head>
