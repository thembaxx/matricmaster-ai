...
import { enrollInSubjectAction, getEnrolledSubjectsAction, getSubjectsAction } from '@/lib/db/actions';
	...
	return (
		<div className="container mx-auto max-w-6xl px-4 pt-8 pb-32">
			<div className="flex items-center gap-4 mb-8">
				...
				<div>
					<h1 className="text-4xl font-black tracking-tight text-foreground uppercase">Subject Marketplace</h1>
			...
			<div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
				{allSubjects.map((subject) => {
					...
					return (
						<Card key={subject.id} className="shadow-tiimo border-border/50 overflow-hidden flex flex-col">
							<CardHeader className="bg-muted/30">
								<CardTitle className="text-xl font-black uppercase tracking-tight">{subject.name}</CardTitle>
								<CardDescription className="text-xs font-bold uppercase tracking-wider">{subject.curriculumCode}</CardDescription>
							</CardHeader>
							<CardContent className="p-6 flex-1 flex flex-col gap-6">
								...
								<div className="mt-auto">
									{isEnrolled ? (
										<Button variant="outline" className="w-full rounded-full gap-2 text-success border-success/30 bg-success/5" disabled>
