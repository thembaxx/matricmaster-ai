...
export function DiscoverTab({
	...
	const getMatchInfo = (buddyId: string) => {
		return weakAreaMatches.find((m) => m.buddy.id === buddyId);
	};
	return (
		<>
			<Card>
			...
			<div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
				{filteredBuddies.map((buddy) => {
					const matchInfo = getMatchInfo(buddy.id);
					const hasWeakAreaMatch = matchInfo && matchInfo.matchScore > 0;
					...
					return (
						<Card key={buddy.id} className="hover:shadow-lg transition-shadow">
									...
									<div className="flex-1">
										<div className="flex items-center gap-2">
											...
											{hasWeakAreaMatch && (
												<Badge
													variant="default"
													className="bg-gradient-to-r from-amber-500 to-primary text-xs gap-1"
												>
							...
							<CardContent>
								...
								{hasWeakAreaMatch && matchInfo.weakAreasTheyCanHelp.length > 0 && (
									<div className="mb-4 p-2 rounded-lg bg-primary/5 border border-primary/10">
										<p className="text-xs font-medium text-primary mb-1">Can help you with:</p>
										<div className="flex flex-wrap gap-1">
											{matchInfo.weakAreasTheyCanHelp.slice(0, 3).map((topic) => (
												<Badge key={topic} variant="outline" className="text-xs bg-background">
													{topic}
												</Badge>
											))}
											{matchInfo.weakAreasTheyCanHelp.length > 3 && (
