...
export default function DashboardWithReactQuery({
	...
	// Parallel queries using useQueries hook
	const [progressQuery, streakQuery] = useQueries({
		queries: [
			{
				queryKey: ['user-progress', userId],
				queryFn: () => fetch('/api/progress').then((res) => res.json()),
				enabled: !!userId,
			...
			},
			{
				...
				queryFn: () => fetch('/api/streak').then((res) => res.json()),
				enabled: !!userId,
	...
	// Use query results or fallback to initial data
	const currentProgress = progressQuery.data || progressData;
	const currentStreak = streakQuery.data || initialStreak;
	...
	const isLoading = isPending || progressQuery.isLoading || streakQuery.isLoading;
	const isError = progressQuery.isError || streakQuery.isError;

	if (isError) {
		// Fallback to initial data if available, otherwise show skeleton
		if (!progressData) {
				...
				>
					...
					<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
						...
						<div className="md:col-span-2 lg:col-span-1">
							<StatsCards
								streak={currentStreak?.currentStreak || streak}
