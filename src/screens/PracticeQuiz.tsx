...
const arithmeticSymbols = ['+', '-', '×', '÷', '=', '<', '>', '±', '%'].map(s => ({ label: s, value: s }));
const greekSymbols = ['α', 'β', 'γ', 'δ', 'ε', 'θ', 'λ', 'μ', 'π', 'σ', 'ω', 'Δ'].map(s => ({ label: s, value: s }));
        ...
        <div className="flex flex-col h-full bg-[#f8f9fb] dark:bg-[#0a0f18] font-inter">
                    ...
                    <div className="space-y-4">
                        ...
                        <Card className="p-8 bg-white dark:bg-zinc-900 border-none shadow-sm rounded-[2.5rem] relative overflow-hidden flex items-center justify-center min-h-[240px]">
                            ...
                            <svg width="240" height="160" viewBox="0 0 240 160" className="relative z-10">
                                {/* Axes */}
                                <line x1="20" y1="140" x2="220" y2="140" stroke="currentColor" strokeWidth="1" className="text-zinc-300 dark:text-zinc-700" />
                                <line x1="40" y1="20" x2="40" y2="140" stroke="currentColor" strokeWidth="1" className="text-zinc-300 dark:text-zinc-700" />
                                ...
                                />
                                {/* Tick labels */}
                                <text x="35" y="155" fontSize="10" className="fill-zinc-400 font-bold">0</text>
                                <text x="135" y="155" fontSize="10" className="fill-zinc-400 font-bold">π</text>
                                <text x="160" y="60" fontSize="11" className="fill-zinc-400 italic">y = sin(x)</text>
                            </svg>
                        </Card>
                    </div>
            ...
            <div className="fixed bottom-0 left-0 right-0 p-6 flex justify-center z-40">
                <Sheet>
                    ...
                    <SheetContent side="bottom" className="h-[520px] rounded-t-[3rem] p-0 border-none bg-[#f1f5f9] dark:bg-[#0a0f18] focus-visible:outline-none overflow-hidden">
                        ...
                        <Tabs defaultValue="calculus" className="w-full flex flex-col h-full">
                            ...
                            <div className="flex-1 px-4 py-6">
                                <AnimatePresence mode="wait">
                                    <TabsContent value="calculus" className="grid grid-cols-4 gap-3 mt-0 h-full content-start">
                                        {calculusSymbols.map((item) => (
                                            <CalcKey key={item.label} label={item.label} onClick={() => handleKeyClick(item.value)} />
                                        ))}
                                        <CalcKey label={<ChevronLeft className="w-6 h-6" />} onClick={() => moveCursor('left')} className="bg-zinc-200 dark:bg-zinc-800/50" />
                                        <CalcKey label={<ChevronRight className="w-6 h-6" />} onClick={() => moveCursor('right')} className="bg-zinc-200 dark:bg-zinc-800/50" />
                                        <Button
                                    ...
                                    <TabsContent value="arithmetic" className="grid grid-cols-4 gap-3 mt-0 h-full content-start">
...
function CalcKey({ label, onClick, className = '' }: { label: React.ReactNode, onClick: () => void, className?: string }) {
    return (
        <motion.button
