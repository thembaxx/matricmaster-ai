...
export default function CMS({ onNavigate }: CMSProps) {
    ...
    const handleSave = () => {
        ...
        if (editingItem.id) {
            setItems(prev => prev.map(item => item.id === editingItem.id ? { ...item, ...editingItem } as ContentItem : item));
        } else {
            ...
            <main className="flex-1 overflow-y-auto no-scrollbar p-6 space-y-4">
                {filteredItems.length === 0 ? (
                    <div className="flex flex-col items-center justify-center py-20 text-center opacity-40">
                        <span className="material-symbols-rounded text-6xl mb-4 text-zinc-300">history_edu</span>
                        <p className="text-sm font-bold">No items found</p>
                    </div>
                ) : (
                    filteredItems.map(item => (
                        <div key={item.id} className="bg-white dark:bg-zinc-900 p-4 rounded-3xl border border-zinc-200 dark:border-zinc-800 shadow-sm flex items-center gap-4 group">
                            {item.image ? (
                                <img src={item.image} className="w-16 h-16 rounded-2xl object-cover bg-zinc-100" alt={item.title} />
                            ) : (
                            ...
                            <div className="flex-1 min-w-0">
                                <div className="flex items-center gap-2 mb-0.5">
                                    <span className={`text-[8px] font-black uppercase tracking-widest px-1.5 py-0.5 rounded-full ${item.status === 'Published' ? 'bg-green-100 text-green-600' : 'bg-orange-100 text-orange-600'}`}>{item.status}</span>
                                    <span className="text-[8px] font-bold text-zinc-400">{item.updatedAt}</span>
                                </div>
                                <h4 className="font-bold text-zinc-900 dark:text-white truncate text-sm">{item.title}</h4>
                                <div className="flex items-center gap-1 text-[10px] text-zinc-500">
                                    <span className="bg-zinc-100 dark:bg-zinc-800 px-1.5 py-0.5 rounded">{item.category}</span>
                                    <span>•</span>
                    ...
                    <div className="bg-white dark:bg-zinc-900 w-full h-[90%] rounded-t-[3rem] flex flex-col overflow-hidden animate-in slide-in-from-bottom duration-300">
                        ...
                        <div className="flex-1 overflow-y-auto no-scrollbar p-6 space-y-6">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    <label className="text-[10px] font-black uppercase text-zinc-400 tracking-widest block mb-2">Title</label>
                                    <input 
                                ...
                                <div>
                                    <label className="text-[10px] font-black uppercase text-zinc-400 tracking-widest block mb-2">Topic</label>
                                    <input 
                            ...
                            {/* Image Upload */}
                            <div>
                                <label className="text-[10px] font-black uppercase text-zinc-400 tracking-widest block mb-2">Featured Image</label>
                                <div 
                                ...
                                >
                                    {editingItem.image ? (
                                        ...
                                        <>
                                            <span className="material-symbols-rounded text-zinc-300 text-3xl mb-1">upload_file</span>
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    <label className="text-[10px] font-black uppercase text-zinc-400 tracking-widest block mb-2">Category</label>
                                    <select 
                                ...
                                <div>
                                    <label className="text-[10px] font-black uppercase text-zinc-400 tracking-widest block mb-2">Difficulty</label>
                                    <select 
                            ...
                            {/* Content Editor */}
                            <div>
                                <label className="text-[10px] font-black uppercase text-zinc-400 tracking-widest block mb-2">Lesson Content (Markdown)</label>
                                <textarea 
                                    rows={8}
