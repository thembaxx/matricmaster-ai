...
export function useAudioPlayer({ audioSrc, text, isOpen, autoPlay = false }: UseAudioPlayerProps) {
		...
		(startText: string, fromIndex = 0) => {
			...
			utterance.onerror = (event) => {
				...
				if (event.error === 'network') {
					...
					setError({ type: 'permission', message: 'Permission denied.', recoverable: false });
				} else {
					setError({ type: 'unknown', message: 'Unable to play audio.', recoverable: true });
				}
			};
		...
		if (audioSrc && audioRef.current && !audioError) {
			if (isPlaying) {
				...
				audioRef.current.play().catch(() => {
					setError({
						type: 'network',
						message: 'Failed to play audio file. Using TTS.',
						recoverable: true,
					});
				});
			}
