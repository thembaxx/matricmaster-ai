...
async function testTwitterAuth() {
	...
	try {
		...
		console.log('  Twitter OAuth:', authConfig.socialProviders?.twitter ? '✅ Configured' : '❌ Not configured');
		
		if (authConfig.socialProviders?.twitter) {
			console.log('\nTwitter Configuration Details:');
			console.log('  Client ID Present:', !!process.env.TWITTER_CLIENT_ID && process.env.TWITTER_CLIENT_ID !== 'your_twitter_client_id_here');
			console.log('  Client Secret Present:', !!process.env.TWITTER_CLIENT_SECRET && process.env.TWITTER_CLIENT_SECRET !== 'your_twitter_client_secret_here');
		}
