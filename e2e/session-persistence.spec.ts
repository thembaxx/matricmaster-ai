...
test.describe('Session Persistence', () => {
    test('User session persists across reloads and has correct expiry', async ({ page, context }) => {
        ...
        if (url.includes('/sign-in')) {
            throw new Error('Redirected to sign-in page when accessing /cms - Session lost or unauthorized and kicked out.');
        }
        ...
        if (newUrl.includes('/cms')) {
            ...
            await expect(page.getByText('Questions', { exact: true })).toBeVisible({ timeout: 5000 }).catch(() => {
        ...
        if (sessionCookie) {
            ...
            if (sessionCookie.expires === -1) {
                console.warn('Warning: Cookie is a session cookie (expires on browser close). This might be incorrect for "remember me" behavior if expected.');
